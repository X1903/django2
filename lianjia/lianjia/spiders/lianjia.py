    ...
    def get_message(self, response):
        ...
        for li in house_list:
            try:
                ...
                # 将离地铁站的距离多少米提取出来，切片选取第二个数字是因为第一个是地铁线号，要提出
            ...
            # 将面积数据提取出来
            item['area'] = re.findall(r'(\d+)', li.find('span', class_='meters').get_text().replace('&nbsp', ''))[0]
            item['room_number'] = li.find('span', class_='zone').find('span').get_text().replace('\xa0', '')

            # 取出楼层，因为里面用/分割了多段文字，所以用split提取
            item['floor'] = li.find('div', class_='con').get_text().split('/')[1]

            # 提取房屋朝向，先找到这个标签在提取文字
            item['direction'] = li.find('div', class_='where').find_all('span')[-1].get_text()
